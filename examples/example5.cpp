...
// if efficiency matters, you want to define the character hash so that it takes
// the key 'A' to the hash value of 'T' and so forth.
//
...
// accordingly.
//
#define canonical_hash(fwd, rev) (fwd == rev ? rev : fwd ^ rev)
...
void demo(int k, string input) {
  ...
  for (int j = k; j < input.size(); j++) {
    ...
    reverse.reverse_update(nucleotide_complement(input[j]),
                           nucleotide_complement(input[j - k]));
