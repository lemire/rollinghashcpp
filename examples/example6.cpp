...
void CalcHashes(const std::string &Inp, const int WindowSize,
                KarpRabinHash<> &h1, ThreeWiseHash<> &h2, GeneralHash<> &h3,
                CyclicHash<> &h4, Adler32 &h5) {
  ...
  for (int i = 0; i < Inp.length(); i++) {
    ...
    if (i + 1 >= WindowSize) {
      ...
      printf("%04d %02d %c %c %06x %06x %06x %06x %06x %c %s \n", i, WindowPos,
             InChar, OutChar, h1.hashvalue, h2.hashvalue, h3.hashvalue,
             h4.hashvalue, h5.hashvalue, (Eat) ? '*' : ' ', current.c_str());
