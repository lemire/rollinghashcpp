...
public:
  // myn is the length of the sequences, e.g., 3 means that you want to hash
  // sequences of 3 characters mywordsize is the number of bits you which to
  // receive as hash values, e.g., 19 means that the hash values are 19-bit
  // integers
  ...
  void reset() { hashvalue = 0; }

  // this is a convenience function, use eat,update and .hashvalue to use as a
  ...
  // the hash value is updated to that of a longer string (one where inchar was
  // appended)
  ...
  // this function can be used to update the hash value from the hash value of
  ...
  void update(chartype outchar, chartype inchar) {
    hashvalue = (B * hashvalue + hasher.hashvalues[inchar] -
                 BtoN * hasher.hashvalues[outchar]) &
                HASHMASK;
  }
...
class KarpRabinHashBits {
  ...
  // wordsize is now templated And the masking is only performed if nbits != the
...
public:
  ...
  // receive as hash values, e.g., 19 means that the hash values are 19-bit
  // integers
  KarpRabinHashBits(int myn)
      : hashvalue(0), n(myn), hasher(maskfnc<hashvaluetype>(wordsize)),
        HASHMASK(maskfnc<hashvaluetype>(wordsize)), BtoN(1) {
    for (int i = 0; i < n; ++i) {
      BtoN *= B;
      if (!is_full_word())
  ...
  template <typename T> void mask_value(T &val) const {
...
#undef CONSTIF
  ...
  hashvaluetype hash(char *s) const {
    return hash(static_cast<const char *>(s));
  }
