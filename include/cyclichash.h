...
public:
  ...
  void update(chartype outchar, chartype inchar) {
    ...
    fastleftshiftn(z);
    hashvalue = getfastleftshift1(hashvalue) ^ z ^ hasher.hashvalues[inchar];
  }

  // this is the reverse of the update function.
  // this function can be used to update the hash value from the hash value of
  // ABC[inchar] to the hash value of [outchar]ABC
  ...
  //  same as hash_extend, but with prepending the n-gram with character Y. If X
  //  = "ABC", then X.hash_prepend("D") returns value of "DABC" without changing
  //  the state of X
